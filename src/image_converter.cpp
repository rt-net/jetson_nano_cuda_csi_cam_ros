...
bool imageConverter::Convert(const sensor_msgs::ImageConstPtr& input)
{
  ...
  {
    ROS_ERROR("failed to convert %ux%u image (from %s to %s) with CUDA", mWidth, mHeight,
              imageFormatToStr(input_format), imageFormatToStr(InternalFormat));
    return false;
  }
...
bool imageConverter::Convert(sensor_msgs::Image& msg, imageFormat format, PixelType* imageGPU)
{
  ...
  {
    ROS_ERROR("failed to convert %ux%u image (from %s to %s) with CUDA", mWidth, mHeight,
              imageFormatToStr(InternalFormat), imageFormatToStr(format));
    return false;
  }
